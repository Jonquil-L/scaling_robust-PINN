...
print(f"Soft BC Unified-Net Alpha-Sensitivity Test starting on: {device}")
...
# ---------------------------------------------------------------------------
# 4. Soft BC Unified Solver
# ---------------------------------------------------------------------------
class UnifiedSoftBCSolver:
...
def hybrid_train(solver, adam_epochs=2000, lbfgs_iters=1000):
    params = list(solver.net.parameters())
...
for alpha in alphas:
    ...
    for sys in ['unscaled', 'scaled']:
        ...
        solver = UnifiedSoftBCSolver(sys, alpha, mms)
...
fig.suptitle(r'$\alpha$-Sensitivity (Soft BC Unified-Net, $\omega_{bc}$=1) — $L^2$, $L^\infty$, $H^1$ Norms',
             fontsize=15)
